...
║   ╚══════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝  ╚═╝       ╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝ ╚═════╝    ║
║                                                                                      ║
║   Luna.AI  ★  ULTIMATE COMBINED APEX  ★  GOD MODE · ALL FEATURES · FULLY FUSED    ║
║                                                                                      ║
║   NEW SYSTEMS:                                                                       ║
...
║                                                                                      ║
║   Inherits: ALL editions fused — Supreme + Nexus + FullSpectrum + GodMode + JARVIS  ║
...
if __name__ == "__main__" and os.environ.get("LUNA_ALLOW_LEGACY_ENTRYPOINTS") == "1":
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ★  LUNA.AI ULTIMATE FUSION — INJECTED MODULES FROM ALL EDITIONS  ★
#  Sources: Luna_AI_Supreme.py · Luna_Nexus_Sentient_v5_3.py · j.py · Lun__AI.py
# ═══════════════════════════════════════════════════════════════════════════════

# ─────────────────────────────────────────────────────────────────────────────
#  FUSION CONSTANTS & UTILITIES
# ─────────────────────────────────────────────────────────────────────────────
SELF_BRAIN_FILE  = HOME_DIR / "nexus" / "self_brain.json"
LUNA_NOTES_FILE  = HOME_DIR / "nexus" / "luna_notes.json"
...
_LUNA_UNCERTAINTY = [
    "Hmm, that one's got me thinking.",
    "Interesting query. Let me look that up for you.",
    "That's at the edge of my knowledge. Let me check.",
    "I'm not entirely certain, but let me search.",
]

def _luna_load_json(path: Path, default):
...
def _luna_save_json(path: Path, data):
    """Safe JSON saver (background thread)."""
    def _w():
        try:
            ...
            pass
    threading.Thread(target=_w, daemon=True, name="LunaJSONSave").start()


# ─────────────────────────────────────────────────────────────────────────────
#  SELFBRAIN — TF-IDF Semantic Vector Memory (no external deps, 100% offline)
# ─────────────────────────────────────────────────────────────────────────────
class SelfBrain:
    """
    Luna's own persistent memory engine.
    Stores up to 20,000 Q/A pairs with 192-dim TF-IDF+trigram vectors.
    Learns from every interaction; recalls via cosine similarity.
    """
    MAX_MEMORIES = 20_000
    VECTOR_DIM   = 192

    def __init__(self):
        raw = _luna_load_json(SELF_BRAIN_FILE, {"memories": [], "facts": {}, "topics": {}})
        self.memories: list = raw.get("memories", [])
        self.facts:    dict = raw.get("facts", {})
    ...
    def _hash_idx(self, token: str) -> int:
        return int(hashlib.md5(token.encode("utf-8")).hexdigest(), 16) % self.VECTOR_DIM
        ...
        for tok in tokens:
            ...
            for n in range(3, min(6, len(tok) + 1)):
                for i in range(0, len(tok) - n + 1):
                    vec[self._hash_idx(tok[i:i+n])] += 0.15
    ...
    def _memory_vector(self, memory: dict) -> list:
        vec = memory.get("vec")
        if isinstance(vec, list) and len(vec) == self.VECTOR_DIM: return vec
        merged = f"{memory.get('q', '')} {memory.get('a', '')[:500]}"
        vec = self._vectorize(merged)
        memory["vec"] = vec
        return vec

    def _backfill_vectors(self):
        changed = False
        for memory in self.memories:
            if not isinstance(memory.get("vec"), list) or len(memory.get("vec", [])) != self.VECTOR_DIM:
                memory["vec"] = self._vectorize(f"{memory.get('q', '')} {memory.get('a', '')[:500]}")
                changed = True
    ...
    def recall(self, query: str, top_k: int = 5) -> list:
        ...
        scored = []
        for memory in self.memories:
            merged  = memory.get("q", "") + " " + memory.get("a", "")[:300]
            lexical  = self._tfidf_similarity(query, merged)
            semantic = self._cosine(query_vec, self._memory_vector(memory))
            score    = (lexical * 0.45) + (semantic * 0.55)
            if score > 0.12: scored.append((score, memory))
        scored.sort(reverse=True, key=lambda x: x[0])
    ...
    def learn(self, question: str, answer: str):
        self.memories.append({
            ...
            "a":  answer[:2000],
            "ts": datetime.datetime.now().isoformat(),
            "vec": self._vectorize(f"{question} {answer[:500]}"),
        })
    ...
    def answer(self, query: str) -> Optional[str]:
        mems = self.recall(query, top_k=3)
        if not mems: return None
        lines = ["[💾 SelfBrain Memory Recall]\n"]
        for i, m in enumerate(mems, 1):
            lines.append(f"Memory {i}: {m['q'][:100]}")
            lines.append(f"  → {m['a'][:400]}\n")
        return "\n".join(lines)

    def build_context_snippet(self, query: str) -> str:
        mems = self.recall(query, top_k=4)
        if not mems: return ""
        lines = ["[RELEVANT PAST KNOWLEDGE]\n"]
        for m in mems:
            lines.append(f"Past Q: {m['q'][:100]}")
            lines.append(f"Past A: {m['a'][:250]}\n")
        return "\n".join(lines)

    def _save(self):
        _luna_save_json(SELF_BRAIN_FILE, {
            "memories": self.memories, "facts": self.facts, "topics": self.topics,
        })

    def stats(self) -> str:
        return f"{len(self.memories):,} memories | {len(self.facts)} facts | {self.VECTOR_DIM}-dim vectors"


# ─────────────────────────────────────────────────────────────────────────────
#  LUNA OWN BRAIN — 100% Offline Built-in Knowledge Base
# ─────────────────────────────────────────────────────────────────────────────
class LunaOwnBrain:
    """Built-in knowledge: programming, math, science, tech — 100% offline."""
    VERSION = "3.0-ULTIMATE"

    KNOWLEDGE = {
        "python list comprehension": "[expr for item in iterable if condition]\nExample: squares = [x**2 for x in range(10) if x%2==0]",
        "python lambda":             "lambda args: expression\nExample: double = lambda x: x * 2",
        "python decorator":          "@decorator\ndef func(): ...\nCommon: @staticmethod, @classmethod, @property, @wraps",
        "python async await":        "async def func():\n    result = await other()\nasyncio.run(func())\ngather() for concurrent.",
        "python class":              "class MyClass:\n    def __init__(self, x):\n        self.x = x\n    def method(self):\n        return self.x",
        "python error handling":     "try:\n    code()\nexcept ValueError as e:\n    print(e)\nfinally:\n    cleanup()",
        "python file read":          "with open('f.txt', 'r', encoding='utf-8') as f:\n    content = f.read()",
        "python json":               "import json\ndata = json.loads(text)\ntext = json.dumps(data, indent=2)",
        "python regex":              "import re\nre.findall(r'\\d+', text)\nre.sub(r'\\s+', ' ', text)",
        "python sort":               "sorted(lst, key=lambda x: x['name'], reverse=True)",
        "python dictionary":         "d.get('key','default')\nd.update({'new':'item'})\n{k: v for k,v in d.items()}",
        "javascript async await":    "async function f(url) {\n    const res = await fetch(url);\n    return await res.json();\n}",
        "javascript arrow function": "const add = (a, b) => a + b;\nconst square = x => x * x;",
        "javascript array methods":  "arr.map(x=>x*2)  arr.filter(x=>x>0)  arr.reduce((a,b)=>a+b,0)",
        "html structure":            "<!DOCTYPE html>\n<html lang='en'>\n<head><meta charset='UTF-8'><title>T</title></head>\n<body></body>\n</html>",
        "css flexbox":               "display:flex; justify-content:center; align-items:center; gap:16px;",
        "css grid":                  "display:grid; grid-template-columns:repeat(3,1fr); gap:16px;",
        "sql basics":                "SELECT col FROM table WHERE cond ORDER BY col LIMIT 10;\nINNER JOIN other ON t.id=other.t_id;\nGROUP BY col HAVING COUNT(*)>5;",
        "git basics":                "git init/clone  git add .  git commit -m 'msg'\ngit push/pull origin main  git branch  git merge",
        "linux commands":            "ls -la  cd /path  mkdir -p a/b  rm -rf dir  cp src dst  grep -r 'pat' .",
        "powershell basics":         "Get-Process  Stop-Process -Name 'app'  Get-ChildItem  Set-Location C:\\",
        "data structures":           "Array O(1)idx  LinkedList O(1)insert  Stack LIFO  Queue FIFO  HashMap O(1)avg  BinaryTree O(logn)",
        "sorting algorithms":        "Bubble O(n²)  Merge O(nlogn) stable  Quick O(nlogn) avg  TimSort Python default",
        "binary search":             "def bs(arr,t):\n    lo,hi=0,len(arr)-1\n    while lo<=hi:\n        mid=(lo+hi)//2\n        if arr[mid]==t: return mid\n        elif arr[mid]<t: lo=mid+1\n        else: hi=mid-1\n    return -1",
        "what is pi":                "π ≈ 3.14159265358979  Circumference=2πr  Area=πr²",
        "pythagorean theorem":       "a²+b²=c²  In right triangle, c=hypotenuse. 3²+4²=5²",
        "quadratic formula":         "x=(-b±√(b²-4ac))/2a  D=b²-4ac: D>0 two roots, D=0 one root, D<0 complex",
        "ohms law":                  "V=I×R  Power: P=V×I=I²R=V²/R",
        "newtons laws":              "1st: Inertia  2nd: F=ma  3rd: Equal & opposite reaction",
        "speed of light":            "c=299,792,458 m/s ≈ 3×10⁸ m/s  E=mc²",
        "what is ai":                "AI simulates human intelligence. Types: Narrow/General/Super. Subfields: ML, DL, NLP, Vision.",
        "what is machine learning":  "ML: systems learn from data. Supervised/Unsupervised/Reinforcement.",
        "what is blockchain":        "Distributed immutable ledger. Decentralized, cryptographically secured. Bitcoin, smart contracts.",
        "what is api":               "Application Programming Interface. REST: GET/POST/PUT/DELETE. JSON data format.",
        "what is docker":            "Containerization: Image(blueprint)+Container(instance)+Dockerfile(instructions).",
        "windows shortcuts":         "Win+D:Desktop  Win+E:Explorer  Win+R:Run  Win+I:Settings  Alt+Tab:Switch  Win+L:Lock",
        "http status codes":         "200OK 201Created 301Redirect 400BadReq 401Unauth 403Forbidden 404NotFound 500ServerErr",
        "common ports":              "HTTP:80 HTTPS:443 FTP:21 SSH:22 DNS:53 MySQL:3306 Postgres:5432 Redis:6379",
        "tcp vs udp":                "TCP: reliable, ordered, connection-based (HTTP,SSH). UDP: fast, no guarantee (video,games,DNS).",
        "what is luna ai":           (
            "I am Luna.AI — the most advanced personal AI ever created.\n\n"
            "ULTIMATE EDITION includes ALL subsystems:\n"
            "• 🧠 GodModeApexCore + FullSpectrumCore + SentientNexusCore\n"
            "• 🌐 20+ AI providers: GPT-4o, Claude, Gemini, DeepSeek, Groq, Mistral, Cohere…\n"
            "• 💻 Local models: WhiteRabbitNeo, Phi-4, Qwen3, LLaMA, GemmaE4B (no API cost)\n"
            "• 🔭 Tri-Brain synthesis: Gemini 2.5-Flash + GPT-4o + Claude simultaneously\n"
            "• 🧬 KnowledgeGraphMindEngine + GraphCognition + SemanticMemoryVault\n"
            "• 🤖 JARVIS OS control: apps, volume, brightness, shutdown, alarms, notes\n"
            "• 👁 YOLOv8 + MediaPipe + SpatialNexus3D environmental awareness\n"
            "• 🔄 SelfEvolution + SkillForge + AutonomousMaintenance\n"
            "• 🎭 EmotionalIntelligence + BiologicalSimulation + MetacognitiveExecutive\n"
            "• 🎙 Voice TTS/STT with sounddevice (no PyAudio required)\n"
        ),
    }
...
print(df.describe())
print(df.isnull().sum())
summary = df.groupby("col").agg({"val": ["sum","mean","count"]})
print(summary)''',
    }
    ...
    def _match_knowledge(self, query: str) -> Optional[str]:
        q_tok = self._tokenize(query)
        best_score, best_answer = 0.0, None
        for key, answer in self.KNOWLEDGE.items():
            k_tok = self._tokenize(key)
            if not k_tok: continue
            overlap = len(q_tok & k_tok) / len(k_tok)
            if overlap > best_score:
                best_score, best_answer = overlap, answer
        return best_answer if best_score >= 0.5 else None

    def _match_code_template(self, query: str) -> Optional[str]:
        q = query.lower()
        for key, code in self.CODE_TEMPLATES.items():
            kw = set(key.split())
            qw = set(q.split())
            if kw & qw and len(kw & qw) >= min(2, len(kw)):
                return f"```python\n{code}\n```\n\n[From Luna's Built-in Code Library]"
        return None

    def _handle_greeting(self, query: str) -> Optional[str]:
        greet_words = {"hello","hi","hey","sup","howdy","greetings","good morning","good evening","good afternoon"}
        q = query.lower().strip()
        if any(q.startswith(g) or q == g for g in greet_words):
            greets = [
                "Hello! I'm Luna.AI, your ultra-intelligent AI. All systems nominal!",
                "Hey! Luna.AI ULTIMATE EDITION online. 20+ brains standing by.",
                "Greetings! Luna.AI at your service. What can I do for you today?",
            ]
            r = greets[self._resp_idx % len(greets)]
            self._resp_idx += 1
            return r
        return None

    def _handle_identity(self, query: str) -> Optional[str]:
        q = query.lower()
        if any(p in q for p in ["who are you","what are you","introduce yourself","your name"]):
            return self.KNOWLEDGE.get("what is luna ai", "I am Luna.AI — the ultimate AI.")
        return None

    def _handle_time_date(self, query: str) -> Optional[str]:
        q = query.lower()
        if any(w in q for w in ["time","date","today","day","year","month"]):
            now = datetime.datetime.now()
    ...
    def _handle_math(self, query: str) -> Optional[str]:
        q = query.strip()
        if re.search(r"[\d].*[+\-*/^].*[\d]|what is \d", q, re.IGNORECASE):
            mp = re.sub(r"(?:what is|calculate|compute|solve|=\s*\?)", "", q, flags=re.IGNORECASE).strip()
            mp = mp.replace("^", "**").replace("×", "*").replace("÷", "/")
            try:
                result = eval(compile(ast.parse(mp, mode="eval"), "<str>", "eval"),
                              {"__builtins__": {}},
    ...
    def answer(self, query: str) -> Optional[str]:
        if not query or len(query.strip()) < 2: return None
        for handler in [self._handle_greeting, self._handle_identity,
                        self._handle_time_date, self._handle_math,
                        self._match_knowledge, self._match_code_template]:
            result = handler(query)
            if result:
                return f"[🧠 Luna's Own Brain]\n\n{result}"
        return None

    def answer_forced(self, query: str) -> str:
        result = self.answer(query)
        if result: return result
        return (f"[🧠 Luna's Own Brain — Offline Mode]\n\n"
                f"Query: {query[:100]}\n\n"
                f"My built-in knowledge covers programming, algorithms, math, and tech.\n"
                f"Try: /search {query[:40]}  or  /math  or  /code <request>")

    def list_knowledge(self) -> str:
        lines = ["🧠 Built-in Knowledge Topics\n" + "─" * 40]
        for k in self.KNOWLEDGE.keys():
            lines.append(f"  • {k}")
        return "\n".join(lines)

    def stats(self) -> str:
        return (f"{len(self.KNOWLEDGE)} knowledge topics | "
                f"{len(self.CODE_TEMPLATES)} code templates | 100% offline")


# ─────────────────────────────────────────────────────────────────────────────
#  APP LAUNCHER — Windows 30+ App Map + Start Menu search
# ─────────────────────────────────────────────────────────────────────────────
class AppLauncher:
    """Launch any Windows app by name, with fuzzy matching + Start Menu fallback."""
    ...
    _KNOWN = {
        ...
        "discord":       str(Path.home() / r"AppData\Local\Discord\Update.exe"),
        "vscode":        str(Path.home() / r"AppData\Local\Programs\Microsoft VS Code\Code.exe"),
        "steam":         r"C:\Program Files (x86)\Steam\steam.exe",
        "whatsapp":      str(Path.home() / r"AppData\Local\WhatsApp\WhatsApp.exe"),
        "telegram":      str(Path.home() / r"AppData\Roaming\Telegram Desktop\Telegram.exe"),
        "zoom":          str(Path.home() / r"AppData\Roaming\Zoom\bin\Zoom.exe"),
        "teams":         str(Path.home() / r"AppData\Local\Microsoft\Teams\current\Teams.exe"),
        "obs":           r"C:\Program Files\obs-studio\bin\64bit\obs64.exe",
        "winrar":        r"C:\Program Files\WinRAR\WinRAR.exe",
        "7zip":          r"C:\Program Files\7-Zip\7zFM.exe",
        "slack":         str(Path.home() / r"AppData\Local\slack\slack.exe"),
        "terminal":      "wt.exe",
    }

    @classmethod
    def open(cls, query: str) -> str:
        q = query.lower().strip()
        # Exact + fuzzy match against known apps
        best_name = None
        best_score = 0
        q_words = set(q.split())
        for name in cls._KNOWN:
        ...
        if best_name and best_score > 0.4:
            path = cls._KNOWN[best_name]
            try:
                if ":" in path and "\\" not in path:
                    import os as _os; _os.startfile(path)
                else:
                    subprocess.Popen(path, shell=True)
                return f"Opening {best_name}."
            except Exception:
                pass
        # Walk Start Menu
        for d in [
            ...
            for root_dir, _, files in os.walk(d):
                for fname in files:
                    if q in fname.lower():
                        try:
                            os.startfile(os.path.join(root_dir, fname))
                            return f"Opening {Path(fname).stem}."
                        except Exception:
                            pass
        subprocess.Popen(f'explorer "search-ms:query={query}&crumb=location:desktop"', shell=True)
        return f"I searched your system for '{query}'."
    ...
    def web_search(cls, query: str) -> str:
        try:
            ...
            return f"[AppLauncher] Could not open browser: {e}"

    @classmethod
    def youtube_search(cls, query: str) -> str:
        try:
            import urllib.parse
...
# ─────────────────────────────────────────────────────────────────────────────
#  NOTES — Persistent JSON note store
# ─────────────────────────────────────────────────────────────────────────────
class LunaNotes:
    """Persistent notes system with JSON storage."""

    @classmethod
    def _load(cls) -> list:
        return _luna_load_json(LUNA_NOTES_FILE, [])

    @classmethod
    def _save(cls, data: list):
        _luna_save_json(LUNA_NOTES_FILE, data)

    @classmethod
    def add(cls, text: str) -> str:
        notes = cls._load()
        notes.append({"id": len(notes) + 1,
                       "time": datetime.datetime.now().isoformat(),
                       "text": text})
        cls._save(notes)
        return f"Got it. Note saved: '{text}'"

    @classmethod
    def read(cls) -> str:
        notes = cls._load()
        if not notes:
            return "You have no saved notes right now."
        recent = notes[-5:]
        return "Here are your recent notes: " + ". ".join(
            f"Note {n['id']}: {n['text']}" for n in recent
        )
    ...
    @classmethod
    def count(cls) -> str:
        return f"{len(cls._load())} notes saved."
...
# ─────────────────────────────────────────────────────────────────────────────
class LunaReminders:
    """Async reminder and alarm system."""
    _active: list = []

    @classmethod
    def set_reminder(cls, message: str, seconds: int, speak_fn=None) -> str:
        def _fire():
            time.sleep(seconds)
        ...
        t = threading.Thread(target=_fire, daemon=True, name="LunaReminder")
        t.start()
        cls._active.append({"msg": message, "in": seconds})
        unit   = "second" if seconds < 60 else "minute"
        amount = seconds if seconds < 60 else seconds // 60
        return f"Reminder set. I will alert you in {amount} {unit}{'s' if amount != 1 else ''}."

    @classmethod
    def set_alarm(cls, hour: int, minute: int, speak_fn=None) -> str:
        def _fire():
            while True:
                now = datetime.datetime.now()
                if now.hour == hour and now.minute == minute:
                    msg = f"Alarm! It is {hour:02d}:{minute:02d}."
                    print(f"\n[⏰ LUNA ALARM] {msg}")
                    if speak_fn:
                        try: speak_fn(msg)
                        except Exception: pass
                    break
                time.sleep(15)
        threading.Thread(target=_fire, daemon=True, name="LunaAlarm").start()
        ampm = "AM" if hour < 12 else "PM"
        h12  = hour % 12 or 12
        return f"Alarm set for {h12}:{minute:02d} {ampm}."

    @classmethod
    def list_active(cls) -> str:
        if not cls._active:
            return "No active reminders."
        return "Active reminders: " + ", ".join(r["msg"] for r in cls._active)


# ─────────────────────────────────────────────────────────────────────────────
#  INTELLIGENCE ENGINE — Weather, News, Jokes, Definitions, Unit conversion…
# ─────────────────────────────────────────────────────────────────────────────
_LUNA_JOKES = [
    "Why do programmers prefer dark mode? Because light attracts bugs!",
    "I told my AI to cheer me up. It said: Have you tried turning yourself off and on again?",
    "Why was the computer cold? It left its Windows open!",
    "My WiFi password is the square root of negative one. It's imaginary.",
...
class LunaIntelligence:
    """Standalone intelligence utilities: weather, news, jokes, definitions, conversions, etc."""
    ...
    def weather(city: str = "") -> str:
        try:
            ...
            loc = r.get("nearest_area", [{}])[0].get("areaName", [{}])[0].get("value", "")
            return (
                f"{c['temp_C']}°C (feels like {c['FeelsLikeC']}°C), "
                f"{c['weatherDesc'][0]['value']}, humidity {c['humidity']}%, "
                f"wind {c['windspeedKmph']} km/h"
                + (f", in {loc}" if loc else "")
            )
        except Exception:
            return "Weather data unavailable. Please check your internet connection."

    @staticmethod
    def joke() -> str:
        return random.choice(_LUNA_JOKES)

    @staticmethod
    def compliment() -> str:
        return random.choice(_LUNA_COMPLIMENTS)
    ...
    def translate(text: str, target_lang: str = "es") -> str:
        try:
            ...
            with urllib.request.urlopen(url, timeout=5) as resp:
                r = json.loads(resp.read().decode())
            return r.get("responseData", {}).get("translatedText", "Translation unavailable.")
        except Exception:
    ...
    def generate_password(length: int = 16) -> str:
        ...
        try:
            ...
            return f"Generated {length}-char password (copied to clipboard): {pwd}"
        except Exception:
    ...
    def news() -> str:
        try:
            import urllib.request
            with urllib.request.urlopen("https://feeds.bbci.co.uk/news/rss.xml", timeout=4) as resp:
                text = resp.read().decode(errors="replace")
            titles = re.findall(r"<title><!\[CDATA\[(.+?)\]\]>", text)[:4]
            if titles:
                return "BBC Headlines: " + ". ".join(titles[1:])
        except Exception:
            pass
...
# ─────────────────────────────────────────────────────────────────────────────
#  LUNA PERSONALITY — JARVIS-style responses
# ─────────────────────────────────────────────────────────────────────────────
class LunaPersonality:
    _GREET = [
        "Hello! All systems nominal. What can I do for you?",
        "Hey! Luna.AI ULTIMATE at your service. What do you need?",
        "Good to hear from you. How can I help?",
        "Online and ready. Fire away.",
    ]
    _HOW_ARE_YOU = [
        "Running at full capacity, thank you for asking. And yourself?",
        "Excellent. All diagnostics green. How are you doing?",
        "Perfectly operational. What can I help you with?",
    ]
    _THANKS = ["My pleasure.", "Anytime.", "Happy to help.", "That's what I'm here for.", "Of course."]
    _BORED  = [
        "I don't get bored — I'm always learning.",
        "Boredom requires consciousness. I'm just very patient.",
    ]
    ...
    def greeting(cls) -> str:
        ...
        return f"Good {tod}! {random.choice(cls._GREET)}"

    @classmethod
    def how_are_you(cls) -> str:
        return random.choice(cls._HOW_ARE_YOU)

    @classmethod
    def thanks(cls) -> str:
        return random.choice(cls._THANKS)

    @classmethod
    def bored(cls) -> str:
        return random.choice(cls._BORED)


# ─────────────────────────────────────────────────────────────────────────────
#  MASTER BRAIN — Tri-Synthesis (Gemini 2.5-Flash + GPT-4o + Claude)
#  Uses the NEW 2026 google-genai library standard
# ─────────────────────────────────────────────────────────────────────────────
class MasterBrain:
    """
    Tri-synthesis engine: fires Gemini 2.5 Flash + GPT-4o + Claude concurrently,
    then uses Gemini as the Master Synthesizer to combine all three into one
    cohesive best-of-all answer. Uses the NEW 2026 google-genai SDK.
    """

    def __init__(self, gemini_key: str = "", openai_key: str = "", claude_key: str = ""):
        self._gemini_key = gemini_key or os.environ.get("GEMINI_API_KEY", "")
        self._openai_key = openai_key or os.environ.get("OPENAI_API_KEY", "")
        self._claude_key = claude_key or os.environ.get("ANTHROPIC_API_KEY", "")
    ...
    def _lazy_init(self):
        ...
        with self._init_lock:
            ...
            try:
                ...
                # Verify model exists, auto-fall-back if 404
                try:
                    self._gemini_client.models.get(model=self.gemini_model)
                except Exception:
                    for m in self._gemini_client.models.list():
                        if hasattr(m, "supported_actions") and "generateContent" in m.supported_actions:
                            self.gemini_model = m.name
                            break
            except Exception as e:
                nprint("MasterBrain", f"Gemini init failed: {e}", C.YELLOW)

            try:
                from openai import OpenAI as _OAI
                self._openai_client = _OAI(api_key=self._openai_key)
            except Exception as e:
                nprint("MasterBrain", f"OpenAI init failed: {e}", C.YELLOW)

            try:
                from anthropic import Anthropic as _Ant
                self._claude_client = _Ant(api_key=self._claude_key)
            except Exception as e:
                nprint("MasterBrain", f"Claude init failed: {e}", C.YELLOW)

            self._initialized = True

    def _call_gemini(self, prompt: str) -> str:
        if not self._gemini_client: return "[Gemini unavailable]"
        try:
    ...
    def _call_gpt(self, prompt: str) -> str:
        if not self._openai_client: return "[GPT unavailable]"
        try:
    ...
    def _call_claude(self, prompt: str) -> str:
        if not self._claude_client: return "[Claude unavailable]"
        try:
    ...
    def synthesize(self, prompt: str, provider: str = "gemini") -> str:
        """Single-provider call."""
        self._lazy_init()
        if provider == "chatgpt":  return self._call_gpt(prompt)
        if provider == "claude":   return self._call_claude(prompt)
        return self._call_gemini(prompt)

    def multi_brain_synthesis(self, prompt: str) -> str:
        """
        Queries Gemini + GPT-4o + Claude concurrently, then uses Gemini
        as the Master Synthesizer to merge all three into one perfect answer.
        """
        self._lazy_init()
        nprint("MasterBrain", f"Tri-Brain synthesis for: '{prompt[:60]}…'", C.MAGENTA)

        with ThreadPoolExecutor(max_workers=3) as ex:
            f_gemini = ex.submit(self._call_gemini, prompt)
            f_gpt    = ex.submit(self._call_gpt,    prompt)
            f_claude = ex.submit(self._call_claude,  prompt)
            r_gemini = f_gemini.result()
            r_gpt    = f_gpt.result()
            r_claude = f_claude.result()

        nprint("MasterBrain", "All three responses gathered. Synthesizing…", C.MAGENTA)

        synthesis_prompt = f"""You are the Master Synthesizer. A user asked:
"{prompt}"

Three advanced AI models responded:
...
Analyze all three responses. Extract the best, most accurate, and most comprehensive
information from all three. Combine them into a single masterfully written, cohesive,
and perfectly accurate final response. Do not mention combining responses; just provide
the final synthesized answer directly."""

        return self._call_gemini(synthesis_prompt)


# ─────────────────────────────────────────────────────────────────────────────
#  LUNA AI ULTIMATE CORE — Final apex class combining ALL editions
# ─────────────────────────────────────────────────────────────────────────────
class LunaAIUltimateCore(GodModeApexCore):
    """
    Luna.AI ULTIMATE COMBINED APEX EDITION
        ...
        NexusCore
          └─ SentientNexusCore
               └─ CapabilityBreakerCore
                    └─ FullSpectrumCore
                         └─ GodModeApexCore
                              └─ LunaAIUltimateCore  ← YOU ARE HERE (ULTIMATE ★)

    NEW in ULTIMATE:
        🔭 MasterBrain      — Gemini 2.5-Flash + GPT-4o + Claude tri-synthesis
        💾 SelfBrain        — TF-IDF semantic vector memory (20,000 Q/As, offline)
        🧠 LunaOwnBrain     — 40+ built-in knowledge topics (100% offline)
        🚀 AppLauncher      — 30+ Windows apps with fuzzy match + Start Menu search
        📝 LunaNotes        — Persistent JSON note store (/note, /notes, /clearnotes)
        ⏰ LunaReminders    — Async reminders and alarms (/remind, /alarm)
        🌡  LunaIntelligence — Weather, news, jokes, definitions, unit conversion,
                               password gen, hash, public IP, translation
        🎭 LunaPersonality  — JARVIS-style personality responses

    New /commands:
        /tribrain <query>    Tri-synthesis: Gemini+GPT+Claude combined answer
        /selfbrain           SelfBrain memory stats
        /selfbrain recall <q> Recall from personal memory
        /ownbrain            Luna's Own Brain knowledge list
        /ownbrain <q>        Query built-in knowledge base
        /app <name>          Launch any Windows application
        /open <name>         Alias for /app
        /google <q>          Google search in browser
        /youtube <q>         YouTube search in browser
        /note <text>         Save a note
        /notes               Read recent notes
        /clearnotes          Clear all notes
        /remind <sec> <msg>  Set reminder in N seconds
        /alarm <HH:MM>       Set alarm for time
        /weather [city]      Current weather
        /news                Latest BBC headlines
        /joke                Random tech joke
        /compliment          Receive a compliment
        /define <word>       Dictionary definition
        /translate <text>    Translate text (default: Spanish)
        /convert <X u1 to u2> Unit conversion
        /password [len]      Generate secure password
        /hash <text>         SHA-256 hash
        /myip                Your public IP
    """

    ULTIMATE_BANNER = f"""{C.CYAN}{C.BOLD}
  ╔══════════════════════════════════════════════════════════════════════════════╗
  ║   LUNA.AI  ★  ULTIMATE COMBINED APEX EDITION  ★  ALL EDITIONS FUSED       ║
  ║                                                                              ║
  ║   🔭 Tri-Brain     Gemini 2.5-Flash + GPT-4o + Claude simultaneous         ║
  ║   💾 SelfBrain     TF-IDF 192-dim vector memory (20K Q/As, offline)        ║
  ║   🧠 OwnBrain      40+ built-in knowledge topics (100% offline)            ║
  ║   🚀 AppLauncher   30+ Windows apps + fuzzy Start Menu search              ║
  ║   📝 Notes/Reminders/Alarms  Persistent JARVIS-style OS utilities          ║
  ║   🌡  Intelligence  Weather · News · Jokes · Define · Convert · Password    ║
  ║   🧬 + ALL GodModeApexCore subsystems (PRD v1.0 complete)                  ║
  ╚══════════════════════════════════════════════════════════════════════════════╝{C.RESET}"""

    def __init__(self):
        nprint("Ultimate", "Initialising Luna.AI ULTIMATE COMBINED APEX…", C.CYAN)
        super().__init__()
        ...
        self.self_brain     = SelfBrain()
        self.own_brain      = LunaOwnBrain()
        self.app_launcher   = AppLauncher()
        self.intelligence   = LunaIntelligence()

        nprint("Ultimate", "All ULTIMATE fusion subsystems online ✅", C.CYAN)

    # ── Enhanced _call_llm: inject SelfBrain context ─────────────────────────

    async def _call_llm(self, prompt: str, system: str = "") -> str:
        """Inject SelfBrain context before every LLM call, then learn from response."""
        ctx = self.self_brain.build_context_snippet(prompt)
        own = self.own_brain.answer(prompt)
        ...
        def _learn():
            try: self.self_brain.learn(prompt[:300], response[:2000])
            except Exception: pass
        threading.Thread(target=_learn, daemon=True, name="SelfBrainLearn").start()

        return response

    # ── Fusion command routing ────────────────────────────────────────────────
    ...
    async def _route_command(self, query: str, modifier: str = "") -> str:
        ...
        # ── Tri-Brain synthesis ───────────────────────────────────────────────
        if ql.startswith("/tribrain "):
            q_text = raw[10:].strip()
            nprint("Ultimate", "Firing Tri-Brain synthesis…", C.MAGENTA)
            return self.master_brain.multi_brain_synthesis(q_text)

        if ql == "/tribrain":
            return ("[TriBrain] Usage: /tribrain <your question>\n"
                    "Fires Gemini 2.5-Flash + GPT-4o + Claude concurrently, "
                    "then synthesizes the best combined answer.")

        # ── SelfBrain ─────────────────────────────────────────────────────────
        if ql == "/selfbrain":
            return f"[💾 SelfBrain] {self.self_brain.stats()}"

        if ql.startswith("/selfbrain recall "):
            q_text = raw[18:].strip()
            result = self.self_brain.answer(q_text)
            return result or "[SelfBrain] No relevant memories found."

        # ── OwnBrain ──────────────────────────────────────────────────────────
        if ql == "/ownbrain":
            return self.own_brain.list_knowledge()

        if ql.startswith("/ownbrain "):
            q_text = raw[10:].strip()
            return self.own_brain.answer(q_text) or f"[OwnBrain] No built-in knowledge for: {q_text}"

        # ── App Launcher ──────────────────────────────────────────────────────
        if ql.startswith("/app ") or ql.startswith("/open "):
            app_name = re.sub(r"^/(app|open)\s+", "", raw, flags=re.I).strip()
            return AppLauncher.open(app_name)

        if ql.startswith("/google "):
            return AppLauncher.web_search(raw[8:].strip())

        if ql.startswith("/youtube "):
            return AppLauncher.youtube_search(raw[9:].strip())

        if ql.startswith("/url "):
            return AppLauncher.open_url(raw[5:].strip())

        # ── Notes ─────────────────────────────────────────────────────────────
        if ql.startswith("/note "):
            return LunaNotes.add(raw[6:].strip())
        if ql in {"/notes", "/readnotes", "/note"}:
            return LunaNotes.read()
        if ql in {"/clearnotes", "/deletenotes"}:
            return LunaNotes.clear()
        if ql == "/notecount":
            return LunaNotes.count()

        # ── Reminders & Alarms ────────────────────────────────────────────────
        if ql.startswith("/remind "):
            parts = raw[8:].strip().split(" ", 1)
            try:
                secs = int(parts[0])
                msg  = parts[1] if len(parts) > 1 else "Reminder!"
                return LunaReminders.set_reminder(msg, secs)
            except (ValueError, IndexError):
                return "[Reminders] Usage: /remind <seconds> <message>"

        if ql.startswith("/alarm "):
            t_str = raw[7:].strip()
            try:
                h, m = map(int, t_str.split(":"))
                return LunaReminders.set_alarm(h, m)
            except Exception:
                return "[Alarm] Usage: /alarm HH:MM  (24-hour format)"

        if ql in {"/reminders", "/alarms"}:
            return LunaReminders.list_active()

        # ── Intelligence ──────────────────────────────────────────────────────
        if ql.startswith("/weather"):
            city = raw[8:].strip()
            return LunaIntelligence.weather(city)

        if ql in {"/news", "/headlines"}:
            return LunaIntelligence.news()

        if ql in {"/joke", "/jokes"}:
            return LunaIntelligence.joke()

        if ql in {"/compliment"}:
            return LunaIntelligence.compliment()

        if ql.startswith("/define "):
            return LunaIntelligence.define_word(raw[8:].strip())

        if ql.startswith("/translate "):
            parts = raw[11:].strip().split(" to ", 1)
            text  = parts[0].strip()
            lang  = parts[1].strip() if len(parts) > 1 else "es"
            return LunaIntelligence.translate(text, lang)

        if ql.startswith("/convert "):
            result = LunaIntelligence.convert_units(raw[9:].strip())
            return result or "[Convert] Usage: /convert 5 km to miles"

        if ql.startswith("/password"):
            parts = raw.split()
            length = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 16
            return LunaIntelligence.generate_password(length)

        if ql.startswith("/hash "):
            return LunaIntelligence.hash_text(raw[6:].strip())

        if ql in {"/myip", "/ip", "/publicip"}:
            return LunaIntelligence.public_ip()

        # ── Help (append ultimate commands) ──────────────────────────────────
        if ql in {"/help", "help"}:
            parent_help = await super()._route_command(query, modifier)
            return self._ultimate_help() + "\n\n" + parent_help

        # ── Delegate up the full chain ────────────────────────────────────────
        return await super()._route_command(query, modifier)

    @staticmethod
    def _ultimate_help() -> str:
        return (
            f"{C.CYAN}\n"
            "╔══════════════════════════════════════════════════════════════════════════╗\n"
            "║  LUNA.AI ULTIMATE — Fusion Command Reference                            ║\n"
            "╠══════════════════════════════════════════════════════════════════════════╣\n"
            "║  TRI-BRAIN SYNTHESIS                                                     ║\n"
            "║  /tribrain <query>         Gemini + GPT-4o + Claude combined answer     ║\n"
            "║                                                                          ║\n"
            "║  MEMORY & KNOWLEDGE                                                      ║\n"
            "║  /selfbrain                SelfBrain memory stats                       ║\n"
            "║  /selfbrain recall <q>     Recall from personal memory                  ║\n"
            "║  /ownbrain                 List built-in knowledge topics               ║\n"
            "║  /ownbrain <q>             Query built-in knowledge (offline)            ║\n"
            "║                                                                          ║\n"
            "║  APP LAUNCHER (Windows)                                                  ║\n"
            "║  /app <name>               Launch any app (fuzzy match)                 ║\n"
            "║  /open <name>              Alias for /app                               ║\n"
            "║  /google <query>           Google search in browser                     ║\n"
            "║  /youtube <query>          YouTube search in browser                    ║\n"
            ...
            "║  /clearnotes               Clear all notes                              ║\n"
            "║  /remind <sec> <msg>       Set reminder in N seconds                    ║\n"
            "║  /alarm HH:MM              Set alarm for exact time (24h)               ║\n"
            "║                                                                          ║\n"
            ...
            "║  /define <word>            Dictionary definition                        ║\n"
            "║  /translate <text> [to XX] Translate text                               ║\n"
            "║  /convert <X u1 to u2>     Unit conversion                              ║\n"
            "║  /password [len]           Generate secure password                     ║\n"
            "║  /hash <text>              SHA-256 hash                                 ║\n"
    ...
    async def run(self):
        """Print ultimate banner, run, then clean up on shutdown."""
        print(self.ULTIMATE_BANNER)
        try:
            await super().run()
        finally:
            nprint("Ultimate", "Saving SelfBrain memory…", C.CYAN)
            self.self_brain._save()
            nprint("Ultimate", "✅ Luna.AI ULTIMATE shut down cleanly.", C.CYAN)
...
async def main():                                                  # noqa: F811
    """
    Luna.AI — ULTIMATE COMBINED APEX EDITION (ALL editions fused, PRD v1.0 complete).
    ...
    core = LunaAIUltimateCore()           # ← ULTIMATE COMBINED APEX — ALL EDITIONS FUSED
    await core.run()


# ─────────────────────────────────────────────────────────────────────────────
#  42.  LUNA.AI FINAL INTERFACE LAYER
#       Additive command surface that unifies chat, memory, local LLM, browser,
#       code forge, GUI, vision, gesture control, and IoT behind one name.
# ─────────────────────────────────────────────────────────────────────────────
class LunaAIPersistentMemory:
    """Small, durable SQLite memory layer used by the final Luna.AI shell."""

    def __init__(self, db_path: Optional[Path] = None):
        self.db_path = Path(db_path or (HOME_DIR / "nexus" / "luna_ai_memory.sqlite3"))
    ...
    def _ensure(self):
        with self._connect() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_luna_memory_role ON memories(role)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_luna_memory_created ON memories(created_at)")

    @staticmethod
    def _tokens(text: str) -> set:
        return set(re.findall(r"\b[a-zA-Z0-9_]{2,}\b", (text or "").lower()))

    def remember(self, text: str, role: str = "note", tags: str = "") -> str:
        text = (text or "").strip()
        if not text:
            return "[Memory] Nothing to remember."
        now = datetime.datetime.now().isoformat(timespec="seconds")
        digest = hashlib.sha256(f"{role}\0{tags}\0{text}".encode("utf-8", "ignore")).hexdigest()
        with self._lock:
            with self._connect() as conn:
                existing = conn.execute(
                    "SELECT id, hits FROM memories WHERE digest = ?", (digest,)
                ).fetchone()
                if existing:
                    conn.execute(
                        "UPDATE memories SET updated_at = ?, hits = ? WHERE id = ?",
                        (now, int(existing["hits"] or 0) + 1, int(existing["id"])),
                    )
                    return f"[Memory] Refreshed memory #{existing['id']}."
                cur = conn.execute(
                    """
                    INSERT INTO memories(created_at, updated_at, role, text, tags, digest, hits)
                    VALUES (?, ?, ?, ?, ?, ?, 1)
                    """,
                    (now, now, role[:40], text[:8000], tags[:400], digest),
                )
                return f"[Memory] Saved memory #{cur.lastrowid}."
    ...
    def recall(self, query: str, limit: int = 6) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            with self._connect() as conn:
                rows = conn.execute(
                    """
                    SELECT id, created_at, updated_at, role, text, tags, hits
                    FROM memories
                    ORDER BY id DESC
                    LIMIT 1200
                    """
        ...
        for row in rows:
            text = row["text"] or ""
            tokens = self._tokens(text + " " + (row["tags"] or "") + " " + (row["role"] or ""))
            overlap = len(q_tokens & tokens)
            if query and not overlap:
                continue
            score = overlap * 3.0
            score += min(int(row["hits"] or 0), 25) * 0.05
    ...
    def build_context(self, query: str, limit: int = 4) -> str:
        ...
        if not hits:
            return ""
        lines = ["[Luna.AI Persistent Memory]"]
        for hit in hits:
            snippet = re.sub(r"\s+", " ", hit["text"]).strip()[:500]
            lines.append(f"- ({hit['role']} #{hit['id']}) {snippet}")
        return "\n".join(lines)
    ...
    def stats(self) -> str:
        with self._lock:
            with self._connect() as conn:
                ...
                roles = conn.execute(
                    "SELECT role, COUNT(*) AS n FROM memories GROUP BY role ORDER BY n DESC"
                ).fetchall()
        role_text = ", ".join(f"{r['role']}={r['n']}" for r in roles[:8]) or "none"
        size_kb = self.db_path.stat().st_size / 1024 if self.db_path.exists() else 0
        return (
            "[Luna.AI Memory]\n"
            f"  Entries : {total}\n"
            f"  Roles   : {role_text}\n"
            f"  Store   : {self.db_path}\n"
            f"  Size    : {size_kb:.1f} KB"
        )

    def export_json(self) -> Path:
        export_path = HOME_DIR / "exports" / f"luna_ai_memory_{int(time.time())}.json"
        export_path.parent.mkdir(parents=True, exist_ok=True)
        with self._lock:
            with self._connect() as conn:
                rows = conn.execute(
                    "SELECT id, created_at, updated_at, role, text, tags, hits FROM memories ORDER BY id"
                ).fetchall()
        export_path.write_text(
            json.dumps([dict(row) for row in rows], indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
        return export_path


class LunaAILocalLLMHub:
    """Final local LLM facade. Prefers existing LocalModelBackend, then Ollama."""
    ...
    async def _ollama_chat(self, model: str, prompt: str, system: str = "",
                           max_tokens: int = 2048) -> Optional[str]:
        payload = {
            "model": model,
            "stream": False,
    ...
    async def chat(self, prompt: str, model: str = "", system: str = "") -> Optional[str]:
        prompt = (prompt or "").strip()
        if not prompt:
            return None
        if not model and hasattr(self.core, "local_models"):
            try:
                result = await self.core.local_models.call_best(prompt, system=system)
                if result:
                    return result
            except Exception:
                pass
        ...
        if not chosen:
            return None
        return await self._ollama_chat(chosen, prompt, system=system)

    def status(self) -> str:
        lines = ["[Luna.AI Local LLM Backend]"]
        if hasattr(self.core, "local_models"):
            try:
                lines.append(self.core.local_models.status_report())
            except Exception as exc:
                lines.append(f"  Existing backend status error: {exc}")
        models = self.ollama_models()
        lines.append(f"  Ollama server : {'ready' if models else 'not reachable or no models pulled'}")
        lines.append(f"  Ollama models : {', '.join(models[:12]) if models else 'none'}")
        lines.append("  Pull example  : ollama pull gemma3:4b")
        return "\n".join(lines)


class LunaAIBrowserAutomation:
    """Browser command wrapper around PlaywrightNavigator plus OS fallback."""
    ...
    async def handle(self, raw: str) -> str:
        q = raw.strip()
        ql = q.lower()
        if ql in {"/browser", "/browser status", "/browse"}:
            return (
                "[Luna.AI Browser Automation]\n"
                f"  Playwright : {'ready' if PLAYWRIGHT_OK else 'unavailable'}\n"
                f"  Headless   : {NEXUS_CFG.get('playwright_headless', True)}\n"
                "  Commands   : /browser open|read|search|screenshot|eval"
            )
        if ql.startswith("/browser open "):
            url = self._url(q[14:])
            webbrowser.open(url)
            return f"[Browser] Opened {url}"
        if ql.startswith("/browser read "):
            url = self._url(q[14:])
            return await self.core.navigator.navigate(url)
        if ql.startswith("/browser search "):
            query = q[16:].strip()
            return await self.core.navigator.search(query)
        if ql.startswith("/browser screenshot "):
            url = self._url(q[20:])
            img = await self.core.navigator.screenshot(url)
            if img is None:
                return "[Browser] Screenshot unavailable. Install Playwright and Pillow."
            out = HOME_DIR / "screenshots" / f"browser_{int(time.time())}.png"
            out.parent.mkdir(parents=True, exist_ok=True)
            img.save(out)
            return f"[Browser] Screenshot saved: {out}"
        if ql.startswith("/browser eval "):
            payload = q[14:].strip()
            if "::" not in payload:
                return "Usage: /browser eval <url> :: <javascript>"
            url, js_code = [part.strip() for part in payload.split("::", 1)]
            return await self._eval(self._url(url), js_code)
        return "Usage: /browser open|read|search|screenshot|eval ..."

    async def _eval(self, url: str, js_code: str) -> str:
        if not PLAYWRIGHT_OK:
            return "[Browser] Playwright unavailable."
        try:
            async with async_playwright() as p:
                browser = await p.chromium.launch(
                    headless=NEXUS_CFG.get("playwright_headless", True)
                )
                page = await browser.new_page()
                await page.goto(url, timeout=15000, wait_until="domcontentloaded")
                result = await page.evaluate(js_code)
                await browser.close()
                return f"[Browser Eval]\n{json.dumps(result, indent=2, default=str)[:3000]}"
        except Exception as exc:
            return f"[Browser Eval Error] {type(exc).__name__}: {exc}"


class LunaAIVisionConsole:
    """Screenshot, OCR, webcam snapshot, and existing YOLO context wrapper."""
    ...
    def status(self) -> str:
        return (
            "[Luna.AI Vision]\n"
            f"  PIL screenshots : {'ready' if PIL_OK else 'unavailable'}\n"
            f"  OpenCV camera   : {'ready' if CV2_OK else 'unavailable'}\n"
            f"  YOLO detector   : {'ready' if YOLO_OK else 'unavailable'}\n"
            f"  Current scene   : {getattr(self.core, 'yolo', None).get_context() if hasattr(self.core, 'yolo') else 'n/a'}\n"
            "  Commands        : /vision screen | /vision camera | /vision yolo"
        )

    def screen(self) -> str:
        if not PIL_OK:
            return "[Vision] Pillow/ImageGrab unavailable."
        try:
            img = ImageGrab.grab()
            out = HOME_DIR / "screenshots" / f"screen_{int(time.time())}.png"
            out.parent.mkdir(parents=True, exist_ok=True)
            img.save(out)
            preview = ""
            try:
                ...
                if text:
                    preview = f"\n  OCR preview : {text[:500]}"
            except Exception:
                preview = "\n  OCR preview : unavailable"
            return f"[Vision] Screen captured: {out}{preview}"
        except Exception as exc:
            return f"[Vision] Screen capture failed: {type(exc).__name__}: {exc}"

    def camera(self) -> str:
        if not CV2_OK:
            return "[Vision] OpenCV unavailable."
        cap = None
        try:
            cap = cv2.VideoCapture(0)
            ok, frame = cap.read()
            if not ok:
                return "[Vision] Could not read from camera 0."
            out = HOME_DIR / "screenshots" / f"camera_{int(time.time())}.jpg"
            out.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(out), frame)
            return f"[Vision] Camera snapshot saved: {out}"
        except Exception as exc:
            return f"[Vision] Camera failed: {type(exc).__name__}: {exc}"
        finally:
            if cap is not None:
                cap.release()
    ...
    def handle(self, raw: str) -> str:
        ql = raw.strip().lower()
        if ql in {"/vision", "/vision status"}:
            return self.status()
        if ql in {"/vision screen", "/see screen", "/screenshot ai"}:
            return self.screen()
        if ql in {"/vision camera", "/camera", "/webcam"}:
            return self.camera()
        if ql in {"/vision yolo", "/yolo status"} and hasattr(self.core, "yolo"):
            return self.core.yolo.get_context()
        return "Usage: /vision screen | /vision camera | /vision yolo"


class LunaAIGestureBridge:
    """Direct control and status for the MediaPipe gesture subsystem."""
    ...
    def handle(self, raw: str) -> str:
        ql = raw.strip().lower()
        gesture = getattr(self.core, "gesture", None)
        if ql in {"/gesture", "/gesture status"}:
            running = bool(getattr(gesture, "_running", False)) if gesture else False
            return (
                "[Luna.AI Gesture Control]\n"
                f"  MediaPipe : {'ready' if MEDIAPIPE_OK else 'unavailable'}\n"
                f"  OpenCV    : {'ready' if CV2_OK else 'unavailable'}\n"
                f"  Enabled   : {NEXUS_CFG.get('gesture_enabled')}\n"
                f"  Running   : {running}\n"
                "  Commands  : /gesture start | /gesture stop | /gesture map"
            )
        if ql == "/gesture start":
            if not gesture:
                return "[Gesture] Gesture subsystem not initialized."
            NEXUS_CFG["gesture_enabled"] = True
            gesture.start()
            return "[Gesture] Start requested."
        if ql == "/gesture stop":
            if gesture:
                gesture.stop()
            return "[Gesture] Stop requested."
        if ql == "/gesture map":
            return "\n".join(
                ["[Gesture Map]"]
                + [f"  {name:14s} -> {cmd}" for name, cmd in GestureController.GESTURE_MAP.items()]
            )
        return "Usage: /gesture status | /gesture start | /gesture stop | /gesture map"


class LunaAIIoTBridge:
    """MQTT and HTTP bridge for ESP32, sensors, and local IoT endpoints."""
    ...
    def status(self) -> str:
        data = {}
        mood = getattr(self.core, "mood", None)
        if mood is not None:
            data = dict(getattr(mood, "_iot_data", {}) or {})
        return (
            "[Luna.AI IoT]\n"
            f"  MQTT library : {'ready' if MQTT_OK else 'unavailable'}\n"
            f"  Broker       : {NEXUS_CFG.get('esp32_host')}:{NEXUS_CFG.get('esp32_mqtt_port')}\n"
            f"  Sensor data  : {json.dumps(data, default=str)[:800] if data else 'none yet'}\n"
            "  Commands     : /iot pub <topic> <payload> | /iot get <url> | /iot post <url> <json>"
        )

    def publish(self, topic: str, payload: str) -> str:
        if not MQTT_OK:
            return "[IoT] paho-mqtt unavailable."
        try:
            try:
                ...
                client = mqtt.Client()
            client.connect(
                NEXUS_CFG.get("esp32_host", "127.0.0.1"),
                int(NEXUS_CFG.get("esp32_mqtt_port", 1883)),
                keepalive=20,
            )
            client.publish(topic, payload)
            client.disconnect()
            return f"[IoT] Published to {topic}: {payload[:200]}"
        except Exception as exc:
            return f"[IoT] Publish failed: {type(exc).__name__}: {exc}"
    ...
    def http_get(url: str) -> str:
        ...
        try:
            ...
            return f"[IoT GET {url}]\n{text[:3000]}"
        except Exception as exc:
            return f"[IoT] GET failed: {type(exc).__name__}: {exc}"
    ...
    def http_post(url: str, payload: str) -> str:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                text = resp.read().decode("utf-8", "replace")
            return f"[IoT POST {url}]\n{text[:3000]}"
        except Exception as exc:
            return f"[IoT] POST failed: {type(exc).__name__}: {exc}"

    def handle(self, raw: str) -> str:
        q = raw.strip()
        ql = q.lower()
        if ql in {"/iot", "/iot status", "/sensors"}:
            return self.status()
        if ql.startswith("/iot pub "):
            rest = q[9:].strip()
            parts = rest.split(" ", 1)
            if len(parts) != 2:
                return "Usage: /iot pub <topic> <payload>"
            return self.publish(parts[0], parts[1])
        if ql.startswith("/iot get "):
            return self.http_get(q[9:].strip())
        if ql.startswith("/iot post "):
            rest = q[10:].strip()
            parts = rest.split(" ", 1)
            if len(parts) != 2:
                return "Usage: /iot post <url> <json>"
            return self.http_post(parts[0], parts[1])
        return "Usage: /iot status | /iot pub <topic> <payload> | /iot get <url> | /iot post <url> <json>"


class LunaAIGUIBridge:
    """Lightweight Tkinter command GUI that talks to the active async core."""
    ...
    def status(self) -> str:
        ...
        return (
            "[Luna.AI GUI]\n"
            f"  Tkinter window : {'running' if alive else 'stopped'}\n"
            f"  HUD overlay    : {'enabled' if NEXUS_CFG.get('hud_enabled') else 'disabled'}\n"
            "  Commands       : /gui launch | /hud"
        )

    def launch(self) -> str:
        if self._thread and self._thread.is_alive():
            return "[GUI] Luna.AI GUI is already running."
        self._thread = threading.Thread(target=self._run, daemon=True, name="LunaAIGUI")
        self._thread.start()
        return "[GUI] Luna.AI command GUI launching."
    ...
    def _run(self):
        try:
            ...
            nprint("GUI", f"Tkinter unavailable: {exc}", C.YELLOW)
            return
        ...
        def submit(event=None):
            ...
            entry.delete(0, "end")
            append("You", query)
            loop = getattr(self.core, "_luna_loop", None)
            if loop is None:
                append("Luna.AI", "Core loop is not ready yet.")
                return
            future = asyncio.run_coroutine_threadsafe(self.core.process(query), loop)
            ...
            def done(fut):
                ...
                root.after(0, append, "Luna.AI", response)

            future.add_done_callback(done)

        send = ttk.Button(bottom, text="Send", command=submit)
        send.pack(side="right", padx=(8, 0))
        entry.bind("<Return>", submit)
        append("Luna.AI", "GUI online. Type a command or chat normally.")
        root.mainloop()


class LunaAIFinalCore(LunaAIUltimateCore):
    """
    Luna.AI final layer.

    This layer does not delete the previous Nexus, Sentient, CapabilityBreaker,
    FullSpectrum, GodMode, or Ultimate systems. It sits on top and gives the
    requested features one direct, stable command surface.
    """

    FINAL_BANNER = f"""{C.CYAN}{C.BOLD}
  ╔══════════════════════════════════════════════════════════════════════════╗
  ║   LUNA.AI  FINAL MODEL INTERFACE                                        ║
  ║   Core chat loop | Memory | Local LLM | Browser | Code Forge            ║
  ║   GUI | Vision | Gesture Control | IoT                                  ║
  ╚══════════════════════════════════════════════════════════════════════════╝{C.RESET}"""

    def __init__(self):
        nprint("Luna.AI", "Activating final unified interface layer...", C.CYAN)
        super().__init__()
        self._luna_loop = None
        self.luna_memory = LunaAIPersistentMemory()
        self.luna_llm = LunaAILocalLLMHub(self)
        self.luna_browser = LunaAIBrowserAutomation(self)
        self.luna_vision = LunaAIVisionConsole(self)
        self.luna_gesture = LunaAIGestureBridge(self)
        self.luna_iot = LunaAIIoTBridge(self)
        self.luna_gui = LunaAIGUIBridge(self)
        nprint("Luna.AI", "Final interface layer online.", C.GREEN)

    def _boot_subsystems(self):
        super()._boot_subsystems()
        if "--gui" in sys.argv:
            self.luna_gui.launch()

    async def _call_llm(self, prompt: str, system: str = "") -> str:
        luna_system = (
            "You are Luna.AI, a powerful local-first personal AI. Be accurate, "
            "direct, useful, and honest about hardware or backend limits."
        )
        if system:
            luna_system += "\n" + system
        return await super()._call_llm(prompt, luna_system)
    ...
    async def process(self, query: str) -> str:
        ...
        if not raw:
            return ""
        if raw.startswith("/"):
            return await super().process(raw)

        memory_context = self.luna_memory.build_context(raw)
        enriched = raw if not memory_context else f"{memory_context}\n\n[Current user message]\n{raw}"
        result = await super().process(enriched)
        self.luna_memory.remember(raw, role="user")
        self.luna_memory.remember(result, role="assistant")
        return result
    ...
    async def _route_command(self, query: str, modifier: str = "") -> str:
        ...
        if ql in {"/luna", "/luna status", "/features", "/core", "/core status", "/lastmodel"}:
            return self._feature_status()

        if ql in {"/memory", "/memory status"}:
            return self.luna_memory.stats()
        if ql.startswith("/memory remember "):
            return self.luna_memory.remember(q[17:].strip(), role="note", tags="manual")
        if ql.startswith("/remember "):
            return self.luna_memory.remember(q[10:].strip(), role="note", tags="manual")
        if ql.startswith("/memory recall "):
            return self._format_memory(self.luna_memory.recall(q[15:].strip(), limit=8))
        if ql.startswith("/recall "):
            return self._format_memory(self.luna_memory.recall(q[8:].strip(), limit=8))
        if ql == "/memory export":
            return f"[Memory] Exported to {self.luna_memory.export_json()}"

        if ql in {"/llm", "/llm status", "/local llm"}:
            return self.luna_llm.status()
        if ql == "/llm refresh" and hasattr(self, "local_models"):
            threading.Thread(target=self.local_models._probe_all, daemon=True).start()
            return "[Local LLM] Refresh probe started."
        if ql.startswith("/llm local "):
            prompt = q[11:].strip()
            result = await self.luna_llm.chat(
                prompt,
                system="You are Luna.AI running on a local model backend.",
            )
            return result or "[Local LLM] No local model responded. Start Ollama and pull a model, e.g. ollama pull gemma3:4b."
        if ql.startswith("/llm ollama "):
            rest = q[12:].strip()
            parts = rest.split(" ", 1)
            if len(parts) != 2:
                return "Usage: /llm ollama <model> <prompt>"
            result = await self.luna_llm.chat(parts[1], model=parts[0])
            return result or f"[Local LLM] Model '{parts[0]}' did not respond."

        if ql.startswith("/browser") or ql == "/browse":
            return await self.luna_browser.handle(q)

        if ql in {"/gui", "/gui status"}:
            return self.luna_gui.status()
        if ql in {"/gui launch", "/gui start"}:
            return self.luna_gui.launch()

        if ql.startswith("/vision") or ql in {"/camera", "/webcam", "/see screen", "/screenshot ai"}:
            return self.luna_vision.handle(q)

        if ql.startswith("/gesture"):
            return self.luna_gesture.handle(q)

        if ql.startswith("/iot") or ql == "/sensors":
            return self.luna_iot.handle(q)

        if ql in {"/forge final", "/code forge", "/codeforge final"}:
            return self.codeforge.status()
        if ql.startswith("/forge make "):
            forged = await self.codeforge.craft(q[12:].strip(), self._call_llm)
            return self.codeforge.render(forged, title="[Luna.AI Code Forge]")

        if ql in {"/help", "help"}:
            parent_help = await super()._route_command(query, modifier)
            return self._final_help() + "\n\n" + parent_help

        return await super()._route_command(query, modifier)

    @staticmethod
    def _format_memory(hits: List[Dict[str, Any]]) -> str:
        if not hits:
            return "[Memory] No matching memories found."
        lines = ["[Luna.AI Memory Recall]"]
        for hit in hits:
            text = re.sub(r"\s+", " ", hit["text"]).strip()[:700]
            lines.append(
                f"  #{hit['id']} [{hit['role']}] score={hit['score']:.1f} "
                f"hits={hit['hits']} :: {text}"
            )
        return "\n".join(lines)

    def _feature_status(self) -> str:
        mem_count = "?"
        try:
            with self.luna_memory._connect() as conn:
                mem_count = str(conn.execute("SELECT COUNT(*) AS n FROM memories").fetchone()["n"])
        except Exception:
            pass
        ollama_models = self.luna_llm.ollama_models()
        gesture_running = bool(getattr(getattr(self, "gesture", None), "_running", False))
        return (
            "[Luna.AI Final Feature Matrix]\n"
            "  Name             : Luna.AI\n"
            "  Core chat loop   : online\n"
            f"  Memory           : online ({mem_count} entries)\n"
            f"  Local LLM        : {'online' if ollama_models else 'ready, waiting for Ollama model'}\n"
            f"  Browser          : {'Playwright ready' if PLAYWRIGHT_OK else 'fallback browser only'}\n"
            f"  Code forge       : {'enabled' if NEXUS_CFG.get('codeforge_enabled', True) else 'disabled'}\n"
            f"  GUI              : {'PyQt HUD ready' if PYQT6_OK else 'Tkinter fallback available'}\n"
            f"  Vision           : PIL={PIL_OK}, OpenCV={CV2_OK}, YOLO={YOLO_OK}\n"
            f"  Gesture control  : {'running' if gesture_running else 'available' if MEDIAPIPE_OK else 'unavailable'}\n"
            f"  IoT              : MQTT={'ready' if MQTT_OK else 'unavailable'}\n"
            "\n"
            "  Commands: /memory /llm /browser /forge /gui /vision /gesture /iot /help"
        )

    @staticmethod
    def _final_help() -> str:
        return (
            "[Luna.AI Final Commands]\n"
            "  /luna status                  Unified feature matrix\n"
            "  /memory remember <text>       Save durable memory\n"
            "  /memory recall <query>        Recall durable memory\n"
            "  /memory export                Export memory as JSON\n"
            "  /llm status                   Local model backend status\n"
            "  /llm local <prompt>           Force local/Ollama model response\n"
            "  /llm ollama <model> <prompt>  Call a specific Ollama model\n"
            "  /browser open <url>           Open URL in your browser\n"
            "  /browser read <url>           Read page text with Playwright\n"
            "  /browser search <query>       Search the web with browser automation\n"
            "  /browser screenshot <url>     Save page screenshot\n"
            "  /forge make <request>         Generate validated code\n"
            "  /gui launch                   Open Luna.AI command GUI\n"
            "  /vision screen                Screenshot + OCR preview\n"
            "  /vision camera                Webcam snapshot\n"
            "  /gesture status|start|stop    Gesture control\n"
            "  /iot status                   MQTT/ESP32 sensor bridge\n"
            "  /iot pub <topic> <payload>    Publish MQTT message\n"
        )

    async def run(self):
        self._luna_loop = asyncio.get_running_loop()
        print(self.FINAL_BANNER)
        try:
            await super().run()
        finally:
            self._luna_loop = None
...
# ─────────────────────────────────────────────────────────────────────────────
#  43.  FINAL main() AND ENTRY POINT — activates LunaAIFinalCore
# ─────────────────────────────────────────────────────────────────────────────
async def main():                                                  # noqa: F811
    """Luna.AI final entry point."""
    if "--setup" in sys.argv:
        print("\nLuna.AI — Final Setup")
        print("-" * 65)
        print("Local LLM:")
        ...
        print("  3. Use: /llm local <prompt>")
        print("\nBrowser automation:")
        print("  python -m playwright install chromium")
        print("\nUseful flags:")
        print("  --no-install   Skip dependency auto-install")
        print("  --hud-off      Disable PyQt HUD")
        print("  --no-yolo      Disable YOLO vision loop")
        print("  --no-gesture   Disable gesture controller")
        print("  --gui          Launch Luna.AI GUI at startup")
        print("\nRun:")
        print("  python Luna.AI.py")
        return
